...
GlslFlavor detectGlslFlavor() {
	...
	if (renderer && renderer->getGLVersionMajor() >= 3) return GlslFlavor::ES3;
	return GlslFlavor::ES2;
#else
	return ofIsGLProgrammableRenderer() ? GlslFlavor::GL3 : GlslFlavor::GL2;
#endif
}

namespace {

std::string fragmentHeader(GlslFlavor flavor) {
	switch (flavor) {
	case GlslFlavor::GL2:
		return "#version 120\n"
...
// Spectral mixing, from spectral.js by Ronald van Wijnen (MIT), as embedded in
// p5.brush's blend shader. pow(x, 2.0) calls are written as products so that
// slightly-negative bases (1.0 - R with R > 1) stay well defined on all GPUs.
const char * SPECTRAL_GLSL = R"GLSL(
...
vec3 spectral_xyz_to_srgb(vec3 xyz) {
  vec3 row0 = vec3( 3.2409699419045200, -1.537383177570090, -0.4986107602930030);
  vec3 row1 = vec3(-0.9692436362808790,  1.875967501507720,  0.0415550574071756);
  vec3 row2 = vec3( 0.0556300796969936, -0.203976958888976,  1.0569715142428700);

  float r = dot(row0, xyz);
  float g = dot(row1, xyz);
  float b = dot(row2, xyz);
...
vec3 spectral_mix(vec3 color1, float tintingStrength1, float factor1, vec3 color2, float tintingStrength2, float factor2) {
  ...
  float luminance1 = spectral_reflectance_to_xyz(R1).y;
  float luminance2 = spectral_reflectance_to_xyz(R2).y;

  float R[SPECTRAL_SIZE];

  float concentration1 = factor1 * factor1 * tintingStrength1 * tintingStrength1 * luminance1;
  float concentration2 = factor2 * factor2 * tintingStrength2 * tintingStrength2 * luminance2;
...
vec3 spectral_mix(vec3 color1, vec3 color2, float factor) {
  return spectral_mix(color1, 1.0, 1.0 - factor, color2, 1.0, factor);
}
)GLSL";

const char * BLEND_MAIN_GLSL = R"GLSL(
...
const float EDGE_MIN = 0.05;
const float EDGE_MAX = 0.35;

void main() {
  // Every buffer shares the same pixel grid, with row 0 at the top of the
  // canvas, so one uv addresses the target copy and the mask alike.
  vec2 uv = gl_FragCoord.xy * u_texel;

  vec4 source = TEX(u_source, uv);
  vec4 maskColor = TEX(u_mask, uv);
  ...
  if (!u_isBrush) {
    ...
    for (int i = -2; i <= 2; i += 2) {
      for (int j = -2; j <= 2; j += 2) {
        vec2 neighborUV = uv + vec2(float(i), float(j)) * u_texel;
        float neighborAlpha = TEX(u_mask, neighborUV).a * 15.0;
  ...
  FRAG_COLOR = vec4(spectral_mix(bgColor, pigment.rgb, mixIntensity), 1.0);
}
)GLSL";

const char * STAMP_MAIN_GLSL = R"GLSL(
uniform vec4 u_color;
uniform sampler2D u_tex;
uniform float u_mode;

VARYING_IN float v_alpha;
VARYING_IN vec2 v_corner;

void main() {
  if (u_mode < 0.5) {
    // Soft circle, identical to p5.brush's point-sprite shader: v_corner
    // spans [-1, 1] across the (point-size-clamped) quad.
    vec2 v = v_corner * 0.5;
    float f = length(v);
    float a = fwidth(f);
    f = 1.0 - smoothstep(0.5 - a, 0.5 + a, f);
    if (f < 0.01) discard;
    FRAG_COLOR = vec4(u_color.xyz, v_alpha * f);
  } else {
    // Image / custom tip: white RGB with ink density in alpha.
    float inkAlpha = TEX(u_tex, v_corner * 0.5 + 0.5).a;
    if (inkAlpha < 0.01) discard;
    FRAG_COLOR = vec4(u_color.rgb, inkAlpha * v_alpha);
  }
}
)GLSL";
...
std::string vertexShader(GlslFlavor flavor) {
	...
	case GlslFlavor::GL2:
		return "#version 120\n"
			   ...
			   "void main() {\n"
			   "  gl_Position = u_proj * vec4(gl_Vertex.xy, 0.0, 1.0);\n"
			   "  v_alpha = gl_Vertex.z;\n"
			   "  v_corner = gl_MultiTexCoord0.xy;\n"
			   "}\n";
	case GlslFlavor::GL3:
		return "#version 150\n"
			   "uniform mat4 u_proj;\n"
...
std::string stampFragmentShader(GlslFlavor flavor) {
	return fragmentHeader(flavor) + STAMP_MAIN_GLSL;
}

std::string blendFragmentShader(GlslFlavor flavor) {
	return fragmentHeader(flavor) + SPECTRAL_GLSL + BLEND_MAIN_GLSL;
}
